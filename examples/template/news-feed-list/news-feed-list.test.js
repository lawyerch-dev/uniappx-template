...
const PAGE_PATH = '/examples/template/news-feed-list/news-feed-list'
const DETAIL_PAGE_PATH = 'examples/template/news-feed-list/detail/detail'
...
describe('template-news-feed-list', () => {
  ...
  it('navigates to detail page and back in narrow layout', async () => {
    ...
    page = await waitForCurrentPagePath('examples/template/news-feed-list/news-feed-list')
    await page.waitFor('text')
  ...
  it('shows detail in split view on tablet layout', async () => {
    ...
    if (isTablet) {
      ...
      const currentPage = await program.currentPage()
      expect(currentPage.path).toBe('examples/template/news-feed-list/news-feed-list')
