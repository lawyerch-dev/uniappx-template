const PAGE_PATH = '/examples/API/interceptor/interceptor'
...
describe('interceptor', () => {
  ...
  if (!isMP) {
    ...
    it('no Interceptor', async () => {
      ...
      expect(currentPage.path).toBe('examples/API/interceptor/page1')
      await program.navigateBack()
    })
    ...
    it('addInterceptor', async () => {
      ...
      const currentPage = await program.currentPage()
      expect(currentPage.path).toBe('examples/API/interceptor/page2')
      await program.navigateBack()
    })
  ...
  it('addSwitchTabInterceptor', async () => {
    ...
    await page.waitFor(500)
    const currentPage = await program.currentPage()
    expect(currentPage.path).toBe('examples/tabBar/API')
  })
  ...
  it('removeSwitchTabInterceptor', async () => {
    ...
    await page.waitFor(500)
    const currentPage = await program.currentPage()
    expect(currentPage.path).toBe('examples/tabBar/component')
  })
  }
  ...
  it('preventNavigateTo', async () => {
    ...
    await page.waitFor(500)
    const currentPage = await program.currentPage()
    expect(currentPage.path).toBe('examples/API/interceptor/interceptor')
  })
