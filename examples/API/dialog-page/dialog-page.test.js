...
const FIRST_PAGE_PATH = '/examples/API/dialog-page/dialog-page'
const NEXT_PAGE_PATH = '/examples/API/dialog-page/next-page'
...
describe('dialog page', () => {
  ...
  if (isIos) {
    ...
    screenShotArea.y = 4
    screenShotArea.x = platformInfo.includes('480x800') ? 220 : 336
  ...
  it('open dialog1', async () => {
    ...
    expect(dialogPageRoute).toBe('examples/API/dialog-page/dialog-1')
  });
  ...
  if (isAndroid && !isDom2) {
    it ('open dialogPage in tabBar', async () => {
      const tabPage = await program.reLaunch('/examples/tabBar/API');
