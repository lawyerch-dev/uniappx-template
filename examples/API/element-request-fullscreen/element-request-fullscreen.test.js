...
describe('/examples/API/element-request-fullscreen/element-request-fullscreen', () => {

  if (isWeb || isMP || (isIOS && platformInfo.includes('15.5'))) {
    it('pass', async () => {
      expect(1).toBe(1);
    });
    return;
  ...
  let page;
  beforeAll(async () => {
    page = await program.reLaunch('/examples/API/element-request-fullscreen/element-request-fullscreen')
